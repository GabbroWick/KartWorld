...
func _test_wings() -> void:
	...
	await _steps(25)
	_check(y_before - _kart.global_position.y < 3.0, "released, the kart keeps its nose angle and does not drop (lost %.1f m in 0.4 s)" % (y_before - _kart.global_position.y))
	...
	_check((_kart.global_position - pos_before).length() > 2.5 and _kart.get_speed() > 6.0, "it keeps cruising forward by itself (%.1f m/s)" % _kart.get_speed())
	_check(_kart.get_speed() > _kart.definition.max_speed * 1.2, "in the air it cruises faster than on the ground (%.1f > %.1f m/s)" % [_kart.get_speed(), _kart.definition.max_speed])
	Input.action_press(InputActions.BRAKE)
	await _steps(10)
	_check(_kart.get_speed() > 6.0, "the brake pedal dives, it never reverses in the air (%.1f m/s)" % _kart.get_speed())
	# Nose down to about -45 deg, then hands off: it keeps diving and lands.
	for i in 120:
		await _steps(1)
		if _kart.motor.fly_pitch < -0.8:
			break
	Input.action_release(InputActions.BRAKE)
	var landed := false
	for i in 300:
...
func _test_bump() -> void:
	...
	var apart := _kart.global_position.distance_to(other.global_position)
	_check(apart > 2.4, "karts stop at their bumpers, not at the small sphere (%.1f m apart)" % apart)
	# Side by side: parked karts placed too close are pushed apart too.
	_kart.place(Transform3D(Basis.IDENTITY, Vector3(24.0, 0.3, 20.0)))
	other.place(Transform3D(Basis.IDENTITY, Vector3(24.8, 0.3, 20.0)))
	await _steps(60)
	var side := absf(_kart.global_position.x - other.global_position.x)
	_check(side > 1.4 and side < 5.0, "overlapping karts separate sideways, gently (%.1f m)" % side)
